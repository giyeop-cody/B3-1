...
Redis를 써봤는데 왜 이렇게 빠른지 설명해보라고 하면 막히는 분들이 많습니다. 이유가 있습니다. **내부의 자료구조를 직접 구현해본 사람이 드물기 때문**입니다. 해시맵, 이중 연결 리스트, 힙을 밑바닥부터 짜면서 LRU와 TTL이 어떻게 동작하는지 손으로 확인합니다.
...
- 이중 연결 리스트와 해시맵을 조합하여 **O(1) LRU 추적**이 가능한 이유를 설명할 수 있다.
...
### 4.1. 기본 자료구조 직접 구현 (내장 Key-Value 컬렉션으로 대체 금지)
...
- 해시 함수는 직접 설계해야 한다
- 충돌 해결은 **체이닝 방식**으로 구현해야 한다 (권장: 이중 연결 리스트 재사용)
...
- 성공 시 `OK`
- 메모리 초과 시 LRU 제거를 수행한다 (세부 규칙은 4.3 참조)
...
- Pub/Sub(보너스 5.5)나 커맨드 히스토리 같은 큐 기반 기능의 기반이 된다
...
- 단, "고정 길이 배열/인덱스 접근" 수준의 저장소가 필요하다면 제한적으로 사용할 수 있는 구현 방식을 선택하되, 내장 컬렉션으로 해시맵/캐시를 대체하는 방식은 금지한다 (예: `dict`로 `put`/`get` 구현 금지).

### 구조

- 각 자료구조(해시맵/이중 연결 리스트/힙)는 **독립된 모듈/파일로 분리**한다
